...
impl SignalModel{
    pub fn new(level_mean:f32,
               level_stdv:f32,
               sd_mean:f32,
               sd_stdv:f32)->SignalModel
    {
        SignalModel{level_mean,level_stdv,sd_mean,sd_stdv}
    }
       ///output signal
...
impl Squiggler{
    pub fn new(path:&std::path::Path)->Result<Squiggler,SquigglerError>{
        use self::SquigglerError::{IoError,NotValidModel,NotEnoughData,ParseError};
        ...
        for line in bufreader.lines(){
            ...
            if line.starts_with("kmer"){
                ...
                if let Some(_) = model.insert(kmer,kmermodel){
    ...
    pub fn get_signal_from_path(&self,fasta_path:&std::path::Path)->std::io::Result<Vec<(u32,u32,f32,f32)>>{
        use std::io::prelude::*;
        use std::fs::File;
    ...
    pub fn get_signal_from_fasta(&self,fasta:&str) -> Vec<(u32,u32,f32,f32)>{
        ...
        for index in 0..range{
            res.push(self.model.get(&seq[index..index+self.kmersize]).unwrap().generate());
        }
        res
    }
    ...
    pub fn convert(&self,fasta_path:&str){
        ...
        let signals = match self.get_signal_from_path(fasta_path){
            Ok(ok) => ok,
            Err(_) => {println!("invalid file");vec![]},
        };
...
pub fn parse(path:&std::path::Path)->std::io::Result<Vec<Signal>>{
    let res = BufReader::new(File::open(path)?)
        ...
        .filter_map(|line|{
            let contents:Vec<_> = line.split('\t').collect();
            let base = match contents[1].chars().nth(0){
                Some(c) => c,
                None => return None,
            };
